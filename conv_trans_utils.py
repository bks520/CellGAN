...
class BasicTransBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn1(x)
        # print('BasicTransBlock',out.shape)
...
class BasicTransDecoderBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        out, q_k_attn = self.attn(x2, x1) # 拼接两个不同的特征层

        out = out + residue #残差叠加
        residue = out

        out = self.bn2(out)   #池化
        out = self.relu(out)    #线性激活
        out = self.mlp(out)     #卷积 为什么叫mlp
...
class RelativePositionBias(nn.Module):
    ...
    def forward(self, H, W):
        ...
        # print('next',self.h,self.w)
        # print('next-', H*W, self.h*self.w, self.num_heads)
        # print('next--',relative_position_bias.shape, H//self.h)
        ...
        # print('relative_position_bias_expanded--', relative_position_bias_expanded.shape)  #将矩阵转置（长X宽，hXw,num_head）->（num_head，长X宽,hXw）->0维拓展（1，num_head，长X宽,hXw）
...
class up_block_trans(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True):
        ...
        for i in range(num_block):  # 叠加trans块的block(BasicTransBlock) 上采样时不调用
            block_list.append(attn_block(out_ch, heads, dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos))
    ...
    def forward(self, x1, x2):
        ...
        out = torch.cat([out, x2], dim=1) # 拼接不同feature
        out = self.blocks(out) # 再进行卷积操作
