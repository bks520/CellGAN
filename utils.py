import numpy as np
from sklearn.metrics import roc_auc_score,jaccard_score,precision_recall_fscore_support\
...
def iou_on_batch(masks, pred):
    ...
    for i in range(pred.shape[0]):
        # pred_tmp = pred[i][0].cpu().detach().numpy()  把0去掉改成了下面那样，不知道会不会影响结果
        ...
        # print("rrr",np.max(mask), np.min(mask))
        # print(mask_tmp.shape,pred_tmp.shape)
